...
from coding_agent.agent.planner import FlatPlanner, TreeOfThoughtPlanner
...
class BaseParserTests:
    ...
    def test_parse_json_plan(self, config: AppConfig):
        planner = FlatPlanner(llm=None, config=config)  # type: ignore[arg-type]
        text = """```json
{
  "goal": "Fix auth bug",
  "subtasks": [
...
```"""
    ...
    def test_format_subtasks(self, config: AppConfig):
        result = FlatPlanner._format_subtasks([
    ...
    def test_empty_subtasks_list(self, config: AppConfig):
        plan = FlatPlanner(llm=None, config=config)._parse_plan_response(  # type: ignore[arg-type]
            '{"goal": "Empty", "subtasks": []}',
        )
...
class TestTreeOfThoughtPlanner:
    def test_plan(self, config: AppConfig):
        planner = TreeOfThoughtPlanner(llm=None, config=config)  # type: ignore[arg-type]
